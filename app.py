# -*- coding: utf-8 -*-
"""
🤖 ربات SMS Bomber - نسخه نهایی بدون خواب
دیتابیس داخلی - نیازی به فایل جدا نیست
"""
...
# توکن بات (مستقیم در کد - فقط برای دیپلوی روی رندر)
...
class MemoryDatabase:
    ...
    def __init__(self):
        self.conn = sqlite3.connect(':memory:')  # دیتابیس در RAM
...
def send_request(url, data, headers=None, method="POST"):
    try:
        h = {"User-Agent": get_random_ua(), "Accept": "application/json"}
        if headers:
            h.update(headers)
...
def keep_alive():
    """هر 10 دقیقه یک بار به خودش پینگ میزنه تا نخوابه"""
    while True:
        try:
            # آمار می‌گیره تا بات فعال بمونه
            db.get_stats()
            print("💓 پینگ زنده نگه داشتن - ساعت:", datetime.now().strftime("%H:%M:%S"))
        except:
            pass
